...
print_usage ()
{
	...
	echo "	-azure-vid, --azure-voice-id	specify Azure Speech voice ID. (default voice ID is \"ja-JP-NanamiNeural\")"
	...
	echo ""
	echo "Example6: The following command specifies the geometry of output mp4 files (1080p), the Azure Region, voice ID/pitch, subscription keyfile path to use Azure Speech. Please refer to the following web page to see what kind of voice ID is available. https://learn.microsoft.com/en-us/azure/cognitive-services/speech-service/language-support?tabs=tts#supported-languages"
...
if [ $NS_FLAG -eq 0 ]; then
	...
	COMMAND_LIST="ffmpeg $FFMPEG_LOG_LEVEL -y -loop 1 -i png/image-{}.png -i mp3/{}.mp3 -r $FPS -vcodec libx264 -tune stillimage -pix_fmt yuv420p -shortest -vf $VF_OPTIONS -c:a copy mp4/{}.mp4; echo \"mp4/{}.mp4 has been created.\""
	...
	COMMAND_LIST="ffmpeg $FFMPEG_LOG_LEVEL -y -loop 1 -i png/image-{}.png -i mp3/{}.mp3 -r $FPS -vcodec libx264 -tune stillimage -pix_fmt yuv420p -shortest -c:a copy mp4/{}.mp4; echo \"mp4/{}.mp4 has been created.\""
